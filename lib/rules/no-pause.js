...
const { isCypressCallChained } = require('../cypress-support/called-by-cypress');
...
module.exports = {
  ...
  create(context) {
    ...
    function checkNodeIsCypressCallAndCallsPause (node) {
      if (isCypressCallChained(node) && isCallingPause(node) && !comments.has(node.loc.start.line - 1)) {
